...
class ConvBlock(nn.Module):
	...
	"""
	def __init__(self, dim, n_ch_in, n_ch_out, n_convs, kernel_size=3, bias = True):
...
class Encoder(nn.Module):
	def __init__(self, dim, n_ch_in, n_enc_stages, n_convs_per_stage, n_filters, kernel_size=3, bias = True):
...
class Decoder(nn.Module):
	def __init__(self, dim, n_ch_in,  n_dec_stages, n_convs_per_stage, n_filters, kernel_size=3, bias=True):
...
class UNet(nn.Module):
	def __init__(self, dim, n_ch_in=2, n_ch_out = 2, n_enc_stages=3, n_convs_per_stage=2, n_filters=16, kernel_size=3, res_connection=True, bias=True):
